...
from models.dpt_block import FeatureFusionBlock_custom_own  # noqa
...
class PixelwiseTaskWithDPT(nn.Module):
    ...
    def forward(self, x, img_info):
        out, features = self.dpt(x, image_size=(img_info[0], img_info[1]))
        if self.postprocess:
...
def create_dpt_gaussian_head(net, has_conf=False):
    """
    return PixelwiseTaskWithDPT for given net params
    """
    assert net.dec_depth > 9
...
class DPTOutputAdapter_fix_to_MVSplat(DPTOutputAdapter):
    """
    Adapt croco's DPTOutputAdapter implementation for dust3r:
    remove duplicated weigths, and fix forward for dust3r
    """
